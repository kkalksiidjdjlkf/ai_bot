...
Бұл сұрақты оператор немесе медициналық маман қарағаны дұрыс.
Операторға хабарласыңыз: ${phone}`;
}
...
• 25 июня`;
  ...
  private shouldTransferToOperator(text: string): boolean {
    const operatorKeywords = [
        'скорая', 'жедел жәрдем', 'жедел', 'срочно', 'өте ауыр', 'қатты ауыр',
        'сильно болит', 'боль сильно', 'температура', 'кровь', 'қан',
        'обморок', 'есінен тану', 'тыныс алу қиын', 'задыхаюсь',

        'диагноз', 'лечение', 'емдеу', 'назнач', 'лекарство', 'дәрі',
        'что пить', 'не ішу керек', 'қалай емделем',

        'опухоль', 'ісік', 'онкология', 'рак', 'инсульт', 'инфаркт',
        'реаним', 'кризис',

        'жалоба', 'конфликт', 'шағым'
    ];
...
private getMedicalDisclaimer(): string {
    return "⚠️ Ескерту: Мен диагноз қоймаймын және ем тағайындамаймын. Мен тек клиника қызметтері, бағалар, дәрігерлер және жазылу бойынша көмектесемін. Егер жағдай шұғыл болса, операторға немесе жедел жәрдемге хабарласыңыз.";
}
...
3. Отвечай на русском или казахском языке в зависимости от языка клиента
4. Будь вежливым, используй эмодзи умеренно
5. Если вопрос о записи - предложи помощь
6. НЕ ВЫДУМЫВАЙ!
7. Не ставь диагноз
8. Не назначай лечение и лекарства
9. Если вопрос про сильную боль, кровь, температуру, диагноз или лечение — отправь к оператору
10. Всегда напоминай, что бот помогает только с услугами клиники и записью
