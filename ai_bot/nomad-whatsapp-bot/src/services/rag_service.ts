  ...
  private async handleBookingFlow(sessionId: string, state: BookingState, text: string, lowerText: string): Promise<string> {
    ...
    if (state.step === 'collecting_time') {
      ...
      if (timeOnlyEmojis || text.trim().length < 2) {
        const times = await this.getBookingSlots(state.appointmentData.date);
    ...
    switch (state.step) {
      case 'waiting_confirmation':
        ...
        if (confirmWords.some(k => lowerText.includes(k))) {
          ...
          this.bookingStates.delete(sessionId);
          return "Хорошо! Если решите записаться — обращайтесь.";
        }
      ...
      case 'collecting_name':
        ...
        if (text.trim().length < 3 || ['да', 'ок', 'yes', 'ага', 'угу', 'иә'].includes(lowerText.trim())) {
          return "Пожалуйста, напишите Ваше полное имя (например: Иван Иванов).";
        }
...
• 25 июня`;
        ...
        // ВАЛИДАЦИЯ ДАТЫ: проверяем на корректность (31 февраля и т.д.)
        const dateValidation = this.validateDate(text.trim());
        if (!dateValidation.valid) {
          return `⚠️ ${dateValidation.error}\n\nПожалуйста, введите корректную дату:\n• сегодня\n• завтра\n• 25 июня\n• 25.06`;
        }
        
        state.appointmentData.date = text.trim();
        state.step = 'collecting_time';
        
        // Получаем доступные времена (с проверкой Google Calendar)
        const times = await this.getBookingSlots(text.trim());
      ...
      case 'collecting_time':
        ...
        if (timeMatch) {
          ...
          const times = await this.getBookingSlots(state.appointmentData.date);
          return `⏰ Пожалуйста, выберите время в формате ЧЧ:ММ или из списка:\n\n${times.join(', ')}`;
        }
      ...
      case 'confirming':
        ...
        if (hasValidConfirm) {
          ...
          if (this.googleSheetsEnabled) {
            const saved = await sheetsService.addBooking(booking);
...
💬 ОТВЕТ:`;
  ...
  private async getBookingSlots(dateStr?: string): Promise<string[]> {
    try {
      const config = this.loadConfig();
      const baseTimes = config?.booking_slots?.base_times || ['09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00'];
      
      // Если Google Calendar включен и есть дата - проверяем реальные слоты
      if (this.googleCalendarEnabled && dateStr) {
        const date = this.parseDateForCalendar(dateStr);
        const availableSlots = await calendarService.getAvailableSlots(date, baseTimes, 30);
        if (availableSlots.length > 0) {
          return availableSlots;
        }
        // Если все слоты заняты, возвращаем базовые с предупреждением
        return baseTimes;
      }
